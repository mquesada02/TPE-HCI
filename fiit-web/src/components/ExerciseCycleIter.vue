<template>
      ...
      <template v-slot:default="{items}" >
        <v-row>
          <v-col
            ...
            lg="3"
          >
            <ExerciseCycleCard :cycleIndex="props.cycleIndex" :title="item.raw.title" :img="item.raw.src" :id="item.raw.id"></ExerciseCycleCard>
          </v-col>
        </v-row>
      </template>
  ...
  <script setup>
    import ExerciseCycleCard from '@/components/ExerciseCycleCard.vue';
    import { computed, ref } from 'vue'
    const props = defineProps(['items', 'cycleIndex'])
