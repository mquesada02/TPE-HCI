<template>
    ...
    <v-container>
            <ExerciseCycleIter :cycleIndex="props.cycleIndex" :items="filteredExercises" :key="finished"/>
    </v-container>
    </v-app>
...
<script setup>
    ...
    import ExerciseCycleIter from '@/components/ExerciseCycleIter.vue'
    import { onBeforeMount, ref, provide, watch } from 'vue';

    const props = defineProps(['cycleIndex'])
    ...
    onBeforeMount(async () => {
            ...
            exercises.value.forEach(async (exercise) => {
                    ...
                    myexercises.value.push({ id: exercise.id, name: exercise.name, src: res?.url || 'https://www.thermaxglobal.com/wp-content/uploads/2020/05/image-not-found.jpg', title: exercise.name })
            });
